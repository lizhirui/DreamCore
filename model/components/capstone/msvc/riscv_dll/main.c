...
__declspec(dllexport) uint32_t riscv32_disasm(char *buffer, uint32_t size, uint32_t address, disasm_item **result)
{
    struct platform riscv32 = 
    {
		CS_ARCH_RISCV,
		CS_MODE_RISCV32,
    ...
    cs_err err = cs_open(riscv32.arch, riscv32.mode, &handle);
    ...
    uint32_t count = cs_disasm(handle, riscv32.code, riscv32.size, address, 0, &insn);
    *result = (disasm_item *)malloc(sizeof(disasm_item) * count);
	...
	{
		(*result)[i].address = insn[i].address;
		(*result)[i].size = insn[i].size;
		memcpy(&(*result)[i].bytes, &insn[i].bytes, sizeof(insn[i].bytes));
		memcpy(&(*result)[i].mnemonic, &insn[i].mnemonic, sizeof(insn[i].mnemonic));
		memcpy(&(*result)[i].op_str, &insn[i].op_str, sizeof(insn[i].op_str));
	}
...
__declspec(dllexport) void result_free(void *buffer)
{
    free(buffer);
}
