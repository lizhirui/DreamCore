...
#define SYSTICK_TICK_CONST                      (SYSFREQ / 10 / RT_TICK_PER_SECOND)
 ...
 * 
 */
static void systick_handler(riscv_trap_arg_t arg)
{
    /* Reload systimer */
...
void rt_hw_ticksetup(void)
{
    ...
    riscv_set_trap_int_handler(RISCV_TRAP_INT_MACHINE_TIMER,systick_handler);
    enable_systimer_irq();
    enable_software_irq();
    systimer_start();
}
